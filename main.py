...
BANNED_USERS_FILE = os.environ.get("BANNED_USERS_FILE", "./banned_users.txt")
...
def is_banned(uid_str):
    return uid_str in banned_users
...
def start(message):
    ...
    if is_banned(user_id_str):
        bot.send_message(message.chat.id, "Raalli ahow, akoonkaaga waa la xanibay. Haddii aad rabto inaad la xiriirto maamulaha, isticmaal /contact ama la xiriir admin.")
        return
...
def set_voice(message):
    user_id_str = str(message.from_user.id)
    if is_banned(user_id_str):
        bot.send_message(message.chat.id, "Raalli ahow, akoonkaaga waa la xanibay.")
        return
    choice = message.text
    if "Ubax" in choice:
...
def cmd_ban(message):
    if message.from_user.id != ADMIN_ID:
        bot.send_message(message.chat.id, "Adigu ma lihid oggolaansho inaad dadka xanibto.")
        return
    parts = message.text.split()
    if len(parts) < 2:
        bot.send_message(message.chat.id, "Isticmaal: /ban <user_id>")
        return
    target = parts[1]
    try:
        target_id = str(int(target))
    except Exception:
        bot.send_message(message.chat.id, "Fadlan geli user id sax ah.")
        return
    if target_id in banned_users:
        bot.send_message(message.chat.id, f"User {target_id} hore ayaa loo xanibay.")
        return
    banned_users.add(target_id)
    save_banned_users()
    bot.send_message(message.chat.id, f"User {target_id} waa la xanibay.")
    try:
        bot.send_message(ADMIN_ID, f"User {target_id} waxaa xanibay @{message.from_user.username if message.from_user.username else message.from_user.first_name}")
    except Exception:
        pass
...
def cmd_unban(message):
    if message.from_user.id != ADMIN_ID:
        bot.send_message(message.chat.id, "Adigu ma lihid oggolaansho inaad dadka ka qaado xanibaadda.")
        return
    parts = message.text.split()
    if len(parts) < 2:
        bot.send_message(message.chat.id, "Isticmaal: /unban <user_id>")
        return
    target = parts[1]
    try:
        target_id = str(int(target))
    ...
    if target_id not in banned_users:
        bot.send_message(message.chat.id, f"User {target_id} lama xanibin.")
        return
    banned_users.discard(target_id)
    save_banned_users()
    bot.send_message(message.chat.id, f"User {target_id} waa laga qaaday xanibaadda.")
    try:
        bot.send_message(ADMIN_ID, f"User {target_id} waxaa ka qaaday xanibaadda @{message.from_user.username if message.from_user.username else message.from_user.first_name}")
    except Exception:
        pass
...
def slider_handler(call):
    ...
    if is_banned(uid):
        try:
            bot.answer_callback_query(call.id, "Akaonkaaga waa la xanibay.", show_alert=True)
        except Exception:
            pass
        return
