...
def concat_with_ffmpeg(part_files, output_path):
    ffmpeg_path = shutil.which("ffmpeg")
    if not ffmpeg_path:
        return False
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".txt", dir=DOWNLOADS_DIR) as listfile:
        for p in part_files:
            listfile.write("file '{}'\n".format(p.replace("'", "'\"'\"'")))
        listname = listfile.name
    cmd = [ffmpeg_path, "-y", "-f", "concat", "-safe", "0", "-i", listname, "-c", "copy", output_path]
    try:
...
def binary_concat(part_files, output_path):
    try:
...
def handle_text(message):
    ...
    try:
        ...
        if len(part_files) == 1:
            try:
                os.rename(part_files[0], final_filename)
                part_files = [final_filename]
            except Exception:
                pass
        else:
            try:
                ok = concat_with_ffmpeg(part_files, final_filename)
                if ok:
                    used_ffmpeg = True
                else:
                    ok2 = binary_concat(part_files, final_filename)
                    if not ok2:
                ...
                bot.send_message(message.chat.id, f"Khalad intii la isku dayayay in la isku daro audio: {e}", reply_to_message_id=message.message_id)
                stop_event.set()
                return
        for p in part_files:
            if p != final_filename:
