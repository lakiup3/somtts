...
def forward_pinned_to_user(dest_chat_id, source_id):
    if not source_id:
        return False
    try:
        chat_info = bot.get_chat(source_id)
        pinned = getattr(chat_info, "pinned_message", None)
        if pinned and getattr(pinned, "message_id", None):
            bot.forward_message(dest_chat_id, source_id, pinned.message_id)
            return True
    except Exception:
        return False
    return False
...
def broadcast_message(message):
    ...
    msg_text = message.text.replace("/dir", "").strip()
    if not msg_text:
        bot.reply_to(message, "Fadlan qor fariinta aad rabto inaad dirto. Tusaale: `/dir Maalin wanaagsan`", parse_mode="Markdown")
        return
    ...
    status_msg = bot.reply_to(message, f"🚀 Fariinta waa la bilaabay...\nKu socota: {total} qof")
...
def cmd_rate(message):
    ...
    forward_pinned_to_user(message.chat.id, OTHER_CHANNEL_SOURCE)
