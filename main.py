...
PIN_SOURCE = os.environ.get("PIN_SOURCE", "@somttsbot_tutorial")  # Set to channel username (e.g. "@uoskaa") or chat id string
...
def forward_pinned_to_user(dest_chat_id, user_id):
    if not PIN_SOURCE:
        return False
    try:
...
def start(message):
    ...
    if not user.get("pinned_sent", False):
        try:
            forward_pinned_to_user(message.chat.id, message.from_user.id)
        except:
            pass
    bot.send_message(
        message.chat.id,
...
def handle_text(message):
    ...
    if not user.get("pinned_sent", False):
        try:
            forward_pinned_to_user(message.chat.id, user_id)
            user = get_user_data(user_id)
        except:
            pass
    admin_info = (
