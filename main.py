...
def set_voice(message):
    ...
    choice = message.text
    # Halkan sidoo kale waxaan ku daray reset si hadii qofku qoraal ku doorto codka ay u noqdaan default
    if "Ubax" in choice:
        voice_data = {"name": "so-SO-UbaxNeural", "label": "Ubax"}
        updates = {"voice": voice_data, "pitch": 0, "rate": 0}
    else:
...
def callback_handler(call):
    ...
    try:
        if call.data.startswith("rate_"):
            ...
            if target == "Ubax":
                voice_data = {"name": "so-SO-UbaxNeural", "label": "Ubax"}
                # Halkan: Reset pitch iyo rate to 0
                updates = {"voice": voice_data, "pitch": 0, "rate": 0}
            elif target == "Muuse":
                voice_data = {"name": "so-SO-MuuseNeural", "label": "Muuse"}
                # Halkan: Reset pitch iyo rate to 0
