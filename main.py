...
LETTER_MAP = {
    ...
    "f": "ef", "g": "ji", "h": "eyj", "i": "ay", "j": "jey",
    "k": "key", "l": "el", "m": "em", "n": "en", "o": "ow",
    "p": "pi", "q": "kiyu", "r": "ar", "s": "es", "t": "ti",
    "u": "yu", "v": "vi", "w": "dabal yu", "x": "eks",
    "y": "way", "z": "zi"
}
...
def normalize_models_local(text: str) -> str:
    def repl(match):
        letter = match.group(1).lower()
        number = int(match.group(2))
        letter_spoken = LETTER_MAP.get(letter, letter)
        number_spoken = number_to_somali(number)
        return f"{letter_spoken} {number_spoken}"
    return re.sub(r"\b([A-Za-z])[-\s]?(\d+)\b", repl, text)

def normalize_acronyms_local(text: str) -> str:
    def repl(match):
        word = match.group().lower()
        if word in ENGLISH_WORD_MAP:
            return ENGLISH_WORD_MAP[word]
        return " ".join(LETTER_MAP.get(c, c) for c in word)
    return re.sub(r"\b[A-Za-z]{2,6}\b", repl, text)

def normalize_english_words_local(text: str) -> str:
    words = text.split()
    out = []
    for w in words:
        key = re.sub(r"[^a-zA-Z]", "", w).lower()
        if key in ENGLISH_WORD_MAP:
            out.append(w.lower().replace(key, ENGLISH_WORD_MAP[key]))
        else:
            out.append(w)
    return " ".join(out)

def normalize_for_somali_tts_local(text: str) -> str:
    text = text.lower()
    text = normalize_models_local(text)
    text = normalize_english_words_local(text)
    text = normalize_acronyms_local(text)
...
def build_system_prompt() -> str:
    return (
        "You are a Somali TTS text normalizer. Receive a user text and return a single"
        " normalized Somali text, ready for Somali TTS to read aloud. Do not add explanations,"
        " do not add extra sentences, do not translate meaning. Normalize numbers to Somali words,"
        " expand acronyms as letter pronunciations (AI -> ey ay), convert model tokens like F-35 to"
        " 'ef soddon iyo shan', convert common English names to Somali phonetic forms (apple -> abal),"
        " replace punctuation ?,!,;,: with commas, and only output the normalized text."
    )
...
def normalize_with_groq(raw_text: str) -> str:
    if not groq_client:
        return normalize_for_somali_tts_local(raw_text)
    try:
        messages = [
            {"role": "system", "content": build_system_prompt()},
            {"role": "user", "content": raw_text}
        ]
        response = groq_client.responses.create(model="openai/gpt-oss-120b", input=messages, max_output_tokens=1024)
        normalized = getattr(response, "output_text", None)
        if normalized:
...
def handle_text(message):
    ...
    text = normalize_with_groq(raw_text)
