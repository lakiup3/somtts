...
def chunk_text(text, max_len=320):
    words = text.split()
    if not words:
        return [text]
    chunks = []
    ...
    for w in words:
        if len(w) > max_len:
            ...
            start = 0
            while start < len(w):
                chunks.append(w[start:start+max_len])
                start += max_len
            continue
        if not cur:
            cur = w
...
async def tts_save_chunk(text_chunk, voice_name, rate, pitch, out_path):
    tts = edge_tts.Communicate(text_chunk, voice_name, rate=rate, pitch=pitch)
...
def handle_text(message):
    ...
    parts = chunk_text(text, max_len=320)
    ...
    part_files = []
    final_file = generate_tts_filename(user_id)

    async def make_tts_parts():
        nonlocal part_files
        for i, chunk in enumerate(parts):
            ts = int(time.time()*1000)
            part_path = os.path.join(DOWNLOADS_DIR, f"part_{user_id}_{ts}_{i}.mp3")
            try:
                await tts_save_chunk(chunk, voice_name, rate, pitch, part_path)
                part_files.append(part_path)
            except Exception as e:
                logging.exception("tts chunk error")
                raise

    try:
        asyncio.run(make_tts_parts())

        if len(part_files) == 0:
            bot.send_message(message.chat.id, "Khalad: Ma la soo saarin audio.", reply_to_message_id=message.message_id)
            return

        if len(part_files) == 1:
            os.replace(part_files[0], final_file)
            part_files = []
        else:
            list_file = os.path.join(DOWNLOADS_DIR, f"concat_list_{user_id}_{int(time.time()*1000)}.txt")
            ...
            try:
                ...
                logging.exception("ffmpeg concat failed")
                bot.send_message(message.chat.id, f"Khalad ffmpeg: {e.stderr.decode('utf-8', errors='ignore')}", reply_to_message_id=message.message_id)
                for p in part_files:
                    if os.path.exists(p):
                        try:
                            os.remove(p)
